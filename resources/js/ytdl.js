(function(){
  ...
  drawVideos = function(i) {
    ...
    for (let g in videos) {
      ...
      let inputFrom = $('<input type="text" class="from" size="5" value="0:00" />');
      let inputTo = $('<input type="text" class="to" size="5" value="'+calcDur(videos[g].duration).split(':').map(durationLead).join(':')+'" />');
      let inputDur = $('<span class="duration">'+calcDur(videos[g].duration).split(':').map(durationLead).join(':')+'</span>');
      let inputFromSec = $('<input type="hidden" class="fromSec" value="0" />');
      let inputDurSec = $('<input type="hidden" class="durSec" value="0" />');
      //$(vid).find('.fromto').append(calcDur(videos[g].duration).split(':').map(durationLead).join(':'));
      $(vid).find('.fromto').empty().append(inputFrom).append(' - ').append(inputTo).append(' (').append(inputDur).append(')');
      $(vid).find('.timeSlider').slider({
        ...
        slide: function( event, ui ) {
          let dur = ui.values[ 1 ] - ui.values[ 0 ];
          $('#'+videos[g].id).find('input.from:eq(0)').val(calcDur(ui.values[ 0 ]).split(':').map(durationLead).join(':'));
          $('#'+videos[g].id).find('input.fromSec:eq(0)').val(ui.values[ 0 ]);
          $('#'+videos[g].id).find('input.to:eq(0)').val(calcDur(ui.values[ 1 ]).split(':').map(durationLead).join(':'));
          $('#'+videos[g].id).find('span.duration:eq(0)').text(calcDur(dur).split(':').map(durationLead).join(':'));
          $('#'+videos[g].id).find('input.durSec:eq(0)').val(dur);
          ...
          //   calcDur(ui.values[ 0 ]).split(':').map(durationLead).join(':') + " ("+ui.values[ 0 ]+") - " + calcDur(ui.values[ 1 ]).split(':').map(durationLead).join(':') + " ("+ui.values[ 1 ]+"), duration: " + calcDur(dur).split(':').map(durationLead).join(':') + "("+dur+")"
          // );
        }
      });
      ...
      if (typeof res.res === 'object') {
        ...
        if (res.res.status == 'end')
          $('#' + param.v).find('.progress:eq(0)').hide().stop(true,false).slideUp({
            complete: function() {
              let button = $('<a href="./ytdl/download/'+param.v+'" class="btn btn-small btn-danger mt-2 fw-bold mx-auto d-flex justify-content-center convertBtn" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: 1rem; --bs-btn-font-size: .6rem; width: 10em;" download>Pobierz</a>');
